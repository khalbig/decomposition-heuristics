...
//#define DPS_INFO
//#define DPS_WRITE_BLOCKPROBLEMS
//#define DPS_WRITE_PARTITION
//#define DPS_WRITE_SOL
...
#define HEUR_TIMING           SCIP_HEURTIMING_BEFORENODE | SCIP_HEURTIMING_AFTERNODE | SCIP_HEURTIMING_DURINGLPLOOP
...
struct SCIP_HeurData
{
   ...
   int                   timing;             /**< should the heuristic run before or after the processing of the node?
                                                  (0: before, 1: after, 2: duringlploop)*/
   ...
   )
{
   ...
   for( c = 0; c < nlinking; c++ )
   {
      ...
      if( ((heurtiming & SCIP_HEURTIMING_AFTERNODE) || (heurtiming & SCIP_HEURTIMING_DURINGLPLOOP)) && (linking->hasrhs || linking->haslhs) )
      {
         SCIP_Real sumrhs = 0;
         SCIP_Real sumlhs = 0;
         for( b = 0; b < linking->nblocks; b++ )
      ...
      if( linking->hasrhs )
      {
         ...
         for( b = 0; b < linking->nblocks; b++ )
            sumrhs += linking->currentrhs[b];
         assert(SCIPisFeasEQ(scip, sumrhs, rhs));
      }
      /* check sum of lhs */
      if( linking->haslhs )
      {
         ...
         for( b = 0; b < linking->nblocks; b++ )
            sumlhs += linking->currentlhs[b];
         assert(SCIPisFeasEQ(scip, sumlhs, lhs));
      }
#endif
   ...
   )
{
   ...
   for( c = 0; c < nlinking; c++ )
   {
      ...
      SCIP_CALL( updateV1(scip, blockproblem, linking, &shift, &(*nviolatedblocksrhs)[c], &(*nviolatedblockslhs)[c], &update) );
...
static
SCIP_RETCODE writesol(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_SOL*             sol,                /**< heuristic solution */
   char*                 prefix              /**< add prefix */
   )
{
   ...
   probname2 = strtok(probname, ".");

   /* specify path */
   (void)SCIPsnprintf(filename, SCIP_MAXSTRLEN, "path-to-folder/%s%s.sol", prefix, probname2+5);
   SCIPinfoMessage(scip, NULL, "dps writes solution to %s\n", filename);
   file = fopen(filename, "w");

   SCIPprintSol(scip, sol, file, TRUE);
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   /* only call heuristic once at the root during lp loop */
   if( heurtiming == SCIP_HEURTIMING_DURINGLPLOOP && SCIPgetDepth(scip) == 0 && (SCIPheurGetNCalls(heur) > 0 || SCIPgetNSepaRounds(scip) > 0) )
      return SCIP_OKAY;

   if( (heurtiming & SCIP_HEURTIMING_BEFORENODE) && heurdata->timing == 0 )
   {
#ifdef DPS_INFO
      SCIPinfoMessage(scip, NULL, "Initialize DPS heuristic before node\n");
#endif
   }
   else if( heurtiming & SCIP_HEURTIMING_DURINGLPLOOP && heurdata->timing == 2 )
   {
#ifdef DPS_INFO
      SCIPinfoMessage(scip, NULL, "Initialize DPS heuristic during lp loop\n");
#endif
   }
   else if( heurtiming & SCIP_HEURTIMING_AFTERNODE && heurdata->timing == 1 )
   {
#ifdef DPS_INFO
      SCIPinfoMessage(scip, NULL, "Initialize DPS heuristic after node\n");
#endif
   }
   else
      return SCIP_OKAY;
   ...
   SCIP_CALL( SCIPsetBoolParam(scip, "decomposition/disablemeasures", disablemeasures) );
      ...
      SCIPinfoMessage(scip, NULL, "dps used decomposition:\n%s\n", SCIPdecompPrintStats(decomp, buffer));
#endif
   ...
   SCIP_CALL( initCurrent(scip, linkings, blockproblem, heurtiming, heurdata->nlinking, nblocks, &success) );
   if( !success )
   {
   ...
   *result = SCIP_DIDNOTFIND;
...
#ifdef DPS_WRITE_BLOCKPROBLEMS
   SCIPinfoMessage(scip, NULL, "dps writes blockproblems\n");
   char filename[SCIP_MAXSTRLEN];
   char probname[SCIP_MAXSTRLEN];
   char* probname2;
   ...
   for( b = 0; b < heurdata->nblocks; b++ )
   {
      ...
      /* specify path */
      (void)SCIPsnprintf(filename, SCIP_MAXSTRLEN, "path-to-folder/blockproblems/%s%s_%i.lp", (heurtiming & SCIP_HEURTIMING_AFTERLPNODE) ? "lp_" : "", probname2+5, b);
      SCIPinfoMessage(scip, NULL, "dps writes block to %s\n", filename);
      SCIP_CALL( SCIPwriteOrigProblem(subscip, filename, "lp", FALSE) );
   }
#endif
   ...
   for( k = 0; k < heurdata->maxit; k++ )
   {
      ...
      /* write current partitions to file */
#ifdef DPS_WRITE_PARTITION
      SCIPinfoMessage(scip, NULL, "dps writes partition\n");
      char filename[SCIP_MAXSTRLEN];
      char probname[SCIP_MAXSTRLEN];
      char* probname2;
      FILE* partitionfile;
      ...
      probname2 = strtok(probname, ".");
      /* specify path */
      (void)SCIPsnprintf(filename, SCIP_MAXSTRLEN, "path-to-folder/%s%s", (heurtiming & SCIP_HEURTIMING_AFTERLPNODE) ? "lp_" : "", probname2+5);
      SCIPinfoMessage(scip, NULL, "dps writes to %s\n", filename);

      if( filename != NULL )
      {
         partitionfile = fopen(filename, "a");
      ...
      for( c = 0; c < heurdata->nlinking; c++ )
      {
         for( b = 0; b < linkings[c]->nblocks; b++ )
         {
            if( linkings[c]->hasrhs )
            {
               SCIPmessageFPrintInfo(SCIPgetMessagehdlr(scip), partitionfile,
               "%s %i %i %s %f %f\n",
               SCIPconsGetName(linkings[c]->linkingcons),
               k,
               linkings[c]->blocknumbers[b],
               "rhs",
               SCIPconsGetRhs(scip, linkings[c]->linkingcons, &success),
               linkings[c]->currentrhs[b]);
            }
            if( linkings[c]->haslhs )
            {
               SCIPmessageFPrintInfo(SCIPgetMessagehdlr(scip), partitionfile,
               ...
               linkings[c]->blocknumbers[b],
               "lhs",
               SCIPconsGetLhs(scip, linkings[c]->linkingcons, &success),
               linkings[c]->currentlhs[b]);
            }
         }
      }
      if( filename != NULL )
      {
         assert(partitionfile != NULL);  /*lint !e449*/
      ...
      if( SCIPisZero(scip, allslacksval) )
      {
...
#ifdef DPS_INFO
         SCIPinfoMessage(scip, NULL, "dps found solution with value %g\n", SCIPgetSolOrigObj(scip, newsol));
#endif
         ...
         if( heurdata->reoptimize )
         {
            ...
            {
#ifdef DPS_INFO
               SCIPinfoMessage(scip, NULL, "dps found reoptimized solution with value %g\n", SCIPgetSolOrigObj(scip, improvedsol));
#endif
               SCIP_CALL( SCIPtrySolFree(scip, &improvedsol, TRUE, FALSE, TRUE, TRUE, TRUE, &success) );
               if( success )
               {
#ifdef DPS_WRITE_SOL
                  SCIP_CALL( writesol(scip, newsol, (heurtiming & SCIP_HEURTIMING_AFTERLPNODE) ? "lp_reopt_" : "reopt_") );
#endif
                  SCIPdebugMsg(scip, "Reoptimizing solution successful\n");
#ifdef DPS_INFO
                  SCIPinfoMessage(scip, NULL, "dps successfully found reoptimized solution\n");
#endif
...
#endif
#ifdef DPS_WRITE_SOL
            SCIP_CALL( writesol(scip, newsol, (heurtiming & SCIP_HEURTIMING_AFTERLPNODE) ? "lp_" : "") );
#endif
...
#ifdef DPS_INFO
               SCIPinfoMessage(scip, NULL, "dps successfully found solution\n");
#endif
               *result = SCIP_FOUNDSOL;
            }
         }
      ...
      else
      {
         ...
         {
#ifdef DPS_INFO
            SCIPinfoMessage(scip, NULL, "dps no update possible\n");
#endif
...
#ifdef DPS_INFO
   SCIPinfoMessage(scip, NULL, "dps stopped after %d iterations after %g seconds\n", k, SCIPheurGetTime(heur));
#endif
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddIntParam(scip, "heuristics/" HEUR_NAME "/timing",
      "should the heuristic run before or after the processing of the node? (0: before, 1: after, 2: duringlploop)",
